    ...
    <section class="flex flex-col gap-4">
      ...
      >
        <section class="flex items-center justify-between gap-4">
          ...
          <span class="gray min-w-max text-sm">{{ edu.startYear }} - {{ edu.endYear }}</span>
        </section>
...
const { data: education } = await useAsyncData(
  'education',
  () => queryContent('education').where({ _type: 'json' }).sort({ _file: -1, $numeric: true }).find()) as unknown as { data: Education }
</script>
